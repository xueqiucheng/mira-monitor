...
- **Railway billing** ([lib/cost/providers/railway-billing.ts](lib/cost/providers/railway-billing.ts)): different file from above — this hits `estimatedUsage` GraphQL for cost ingest. Two auth modes auto-detected on first call (`detectRailwayAuthMode`):
  - **Project Token**: header `Project-Access-Token: <token>` + must pass `projectId` arg to `estimatedUsage`. projectId is discovered via `{ projectToken { projectId } }` probe.
  - **Account/Workspace Token**: header `Authorization: Bearer <token>` + `estimatedUsage` returns all visible projects' usage.
  - Both probes failing throws an error listing both attempts. Discovered via 2026-05 Railway auth tightening; see `scripts/test-railway.ts` for the 4-combo probe matrix.
...
- **Apollo's `day` bucket is rolling 24h, not calendar day**: Apollo API returns `{day: {consumed, limit}, hour: {...}, minute: {...}}` with no timestamp; the `day` value is a 24h trailing window. Numbers fluctuate within a single day as new calls enter / old calls leave the window. Our `cost_apollo_daily.beijing_date` row is an UPSERT — by 23:30 last fire of the day, it represents "rolling 24h ending at 23:30 of that calendar day", which is *close* to the calendar day but not exact. UI is labeled "Apollo (近 24h)" and the trend chart's Apollo column has a ⓘ. Don't rename the table without a migration; the misleading "_daily" suffix is intentional for consistency with the other 3 providers.
